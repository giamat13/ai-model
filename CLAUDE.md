...
**[training_data.json](training_data.json) is the single source of truth** for all training data — dialogue, knowledge, arithmetic tool-calls, naming, and code examples all live in one `{"metadata": {...}, "articles": [...]}` file, ~160.8k articles.

> **History:** this used to be split across `math_training_data.json`, `naming_training_data.json`, `new_training_examples.json`, `better_code_examples.json`, and `simple_code.json`. Two of those (`new_training_examples.json`, and `better_code_examples.json`/`simple_code.json` entirely) were **silently never loaded** — a top-level-key mismatch (`new_training_data` instead of `articles`) meant `load_all_articles()` always read 0 items from them, and the other two weren't even in `DATA_PATHS`. All five were folded into `training_data.json` (verified: zero `id` collisions, all ~10.5k items now actually trained on) and deleted. [data_utils.py](data_utils.py)'s `merge_into_training_data()` is now the one way generator scripts add data — merge-by-`id` into the single file, not "write your own JSON".

- **[add_math_data.py](add_math_data.py)** — regenerates arithmetic tool-call exemplars (`calc_*` ids) and merges them in
...
- **[generate_naming_data.py](generate_naming_data.py)** — regenerates creative-naming exemplars (`naming_he_*`/`naming_en_*` ids) and merges them in
- **API-fetched data** (optional; see collect_corpus.py):
  - `api_training_data.json`, `fetched_articles.json` — loaded directly by `train.py` if present (correct `{"articles": [...]}` schema), or merge them into `training_data.json` with `collect_corpus.py --merge-into training_data.json`
  - Not in git (too large); rebuild locally if needed

**Training data format:** `{"id": "...", "text": "..."}` per article. The `id` must be stable across runs (hash-based skipping depends on it) and globally unique across the whole file — `merge_into_training_data()` replaces on `id` match rather than duplicating.
...
1. Everything goes into `training_data.json` — it's the single source of truth:
   - Dialogue/knowledge/code → append directly to its `articles` list (unique `id` + `text`)
   - Arithmetic → run `python add_math_data.py` (regenerates `calc_*` exemplars, merges by `id`)
   - Naming → run `python generate_naming_data.py` (regenerates `naming_*` exemplars, merges by `id`)
   - New generator script → have it call `data_utils.merge_into_training_data(articles, "Label")` instead of writing its own JSON file
