...
with open(os.path.join(_HERE, "simple_code.json"), "r", encoding="utf-8") as f:
    new_data = json.load(f)

with open(os.path.join(_HERE, "training_data.json"), "r", encoding="utf-8") as f:
    main_data = json.load(f)

main_data["articles"].extend(new_data["simple_code"])
...
print(f"Added {len(new_data['simple_code'])} simple examples")
